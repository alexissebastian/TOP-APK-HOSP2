...
public final class h implements com.swmansion.gesturehandler.core.h {
    ...
    private final SparseArray<int[]> f14341a = new SparseArray<>();
    @NotNull
    ...
    public boolean a(@NotNull GestureHandler<?> handler, @NotNull GestureHandler<?> otherHandler) {
        ...
        int[] iArr = this.f14341a.get(handler.O());
        if (iArr != null) {
            for (int i : iArr) {
                if (i == otherHandler.O()) {
                    return true;
    ...
    public boolean d(@NotNull GestureHandler<?> handler, @NotNull GestureHandler<?> otherHandler) {
        Intrinsics.checkNotNullParameter(handler, "handler");
        Intrinsics.checkNotNullParameter(otherHandler, "otherHandler");
        if (otherHandler instanceof com.swmansion.gesturehandler.core.n) {
            return ((com.swmansion.gesturehandler.core.n) otherHandler).K0();
        }
        return false;
    }
    ...
    public final void e(@NotNull GestureHandler<?> handler, @NotNull ReadableMap config) {
        ...
        if (config.hasKey("waitFor")) {
            this.f14341a.put(handler.O(), f(config, "waitFor"));
        }
        if (config.hasKey("simultaneousHandlers")) {
            this.b.put(handler.O(), f(config, "simultaneousHandlers"));
        }
    }
