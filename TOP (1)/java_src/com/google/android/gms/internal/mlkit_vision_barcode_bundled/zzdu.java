...
public class zzdu<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    private final int zza;
    private boolean zzd;
    private volatile zzdt zze;
    private List<zzdr> zzb = Collections.emptyList();
    ...
    public final V zzl(int i) {
        ...
        if (!this.zzc.isEmpty()) {
            ...
            List<zzdr> list = this.zzb;
            Map.Entry<K, V> next = it.next();
            list.add(new zzdr(this, next.getKey(), next.getValue()));
            it.remove();
        }
        return v;
    }
    ...
    public final boolean equals(Object obj) {
        ...
        int size = size();
        if (size != zzduVar.size()) {
            return false;
        }
        int zzb = zzb();
        if (zzb == zzduVar.zzb()) {
            for (int i = 0; i < zzb; i++) {
                if (!zzg(i).equals(zzduVar.zzg(i))) {
                    return false;
                }
            }
            if (zzb != size) {
                return this.zzc.equals(zzduVar.zzc);
            }
            return true;
        }
        return entrySet().equals(zzduVar.entrySet());
    }
    ...
    public final Iterable<Map.Entry<K, V>> zzc() {
        return this.zzc.isEmpty() ? zzdq.zza() : this.zzc.entrySet();
    }
    ...
    public final V put(K k, V v) {
        ...
        int i2 = this.zza;
        if (size == i2) {
            zzdr remove = this.zzb.remove(i2 - 1);
            zzm().put((K) remove.zza(), (V) remove.getValue());
        }
        this.zzb.add(i, new zzdr(this, k, v));
        return null;
    }
