    ...
    public Jwt parse(String str) throws ExpiredJwtException, MalformedJwtException, SignatureException {
        ...
        if (!"..".equals(str)) {
            ...
            if (i2 == 2) {
                        ...
                        if (key == null) {
                            ...
                            if (Objects.isEmpty(bArr) && (signingKeyResolver = this.signingKeyResolver) != null) {
                                if (defaultClaims != null) {
                                    ...
                                    resolveSigningKey = signingKeyResolver.resolveSigningKey(jwsHeader2, str2);
                                }
                                key = resolveSigningKey;
                            }
                ...
                if (defaultClaims != null) {
                    ...
                    if (expiration != null) {
                        ...
                        if ((z ? new Date(time - this.allowedClockSkewMillis) : now).after(expiration)) {
                            throw new ExpiredJwtException(jwsHeader, defaultClaims, "JWT expired at " + DateFormats.formatIso8601(expiration, false) + ". Current time: " + DateFormats.formatIso8601(now, false) + ", a difference of " + (j2 - expiration.getTime()) + " milliseconds.  Allowed clock skew: " + this.allowedClockSkewMillis + " milliseconds.");
                        }
                    } else {
                        claims = str2;
                    }
                    ...
                    if (notBefore != null) {
                        if ((z ? new Date(time + this.allowedClockSkewMillis) : now).before(notBefore)) {
                            throw new PrematureJwtException(jwsHeader, defaultClaims, "JWT must not be accepted before " + DateFormats.formatIso8601(notBefore, false) + ". Current time: " + DateFormats.formatIso8601(now, false) + ", a difference of " + (notBefore.getTime() - j) + " milliseconds.  Allowed clock skew: " + this.allowedClockSkewMillis + " milliseconds.");
                        }
                    }
                ...
                if (sb2 != null) {
                    return new DefaultJws(jwsHeader, defaultClaims2, sb2);
                }
