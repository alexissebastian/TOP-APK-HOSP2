...
public class b {
    ...
    public static Uri a(@NonNull ReactContext reactContext, @NonNull File file) {
        ...
        int i = 0;
        while (true) {
            ArrayList<String> arrayList = f13111a;
            if (i >= arrayList.size()) {
                break;
            }
            try {
    ...
    public static String d(Context context, Uri uri, Boolean bool) {
        ...
        if (Build.VERSION.SDK_INT >= 19 && DocumentsContract.isDocumentUri(context, uri)) {
            if (f(uri)) {
                String[] split = DocumentsContract.getDocumentId(uri).split(OobNotificationProfile.SEPERATOR);
                String str = split[0];
                ...
                String[] split2 = DocumentsContract.getDocumentId(uri).split(OobNotificationProfile.SEPERATOR);
