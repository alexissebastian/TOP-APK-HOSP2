package util.gb;

import android.content.Context;
import android.content.Intent;
import android.content.pm.PackageManager;
import android.content.pm.Signature;
/* loaded from: classes3.dex */
class f {
    public static boolean a(Intent intent, String str) {
        String stringExtra;
        return (intent == null || (stringExtra = intent.getStringExtra("wx_token_key")) == null || !stringExtra.equals(str)) ? false : true;
    }

    public static boolean b(Context context, Signature[] signatureArr, boolean z) {
        String str;
        if (z) {
            for (Signature signature : signatureArr) {
                String lowerCase = signature.toCharsString().toLowerCase();
                util.hb.b.a("MicroMsg.SDK.WXMsgImplComm", "check signature:" + lowerCase);
                if (lowerCase.equals("")) {
                    str = "pass";
                }
            }
            return false;
        }
        str = "ignore wechat app signature validation";
        util.hb.b.a("MicroMsg.SDK.WXMsgImplComm", str);
        return true;
    }

    public static boolean c(Context context, String str, boolean z) {
        if (z) {
            try {
                return b(context, context.getPackageManager().getPackageInfo(str, 64).signatures, z);
            } catch (PackageManager.NameNotFoundException unused) {
                return false;
            }
        }
        util.hb.b.a("MicroMsg.SDK.WXMsgImplComm", "ignore wechat app signature validation");
        return true;
    }
}
